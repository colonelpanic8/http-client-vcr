use crate::filter::{BodyFilter, FilterChain, HeaderFilter};

/// Creates a filter chain specifically designed for Last.fm HTTP interactions
/// This removes/replaces sensitive authentication and session data
pub fn create_lastfm_filter_chain() -> Result<FilterChain, regex::Error> {
    let filter_chain = FilterChain::new()
        ...
        // Filter form data in request bodies (URL-encoded)
        .add_filter(Box::new(
            BodyFilter::new()
                // CSRF tokens in form data
                .replace_regex(
                    r"csrfmiddlewaretoken=[^&]+",
                    "csrfmiddlewaretoken=[FILTERED_CSRF]",
                )?
                // Username/email in form data
                .replace_regex(
                    r"username_or_email=[^&]+",
                    "username_or_email=[FILTERED_USERNAME]",
                )?
                // Password in form data
                .replace_regex(r"password=[^&]+", "password=[FILTERED_PASSWORD]")?
                // Session IDs in form data
                .replace_regex(r"sessionid=[^&;]+", "sessionid=[FILTERED_SESSION]")?,
        ))
        // Filter HTML response bodies containing sensitive data
        .add_filter(Box::new(
            BodyFilter::new()
...
pub fn create_minimal_lastfm_filter_chain() -> Result<FilterChain, regex::Error> {
    let filter_chain = FilterChain::new()
        ...
        // Only filter credentials in form data
        .add_filter(Box::new(
            BodyFilter::new()
                .replace_regex(r"username_or_email=[^&]+", "username_or_email=[FILTERED]")?
                .replace_regex(r"password=[^&]+", "password=[FILTERED]")?,
        ));
